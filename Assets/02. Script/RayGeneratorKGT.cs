...
public class RayGeneratorKGT : MonoBehaviour
{
    [SerializeField]
    ...
    public void RayGenerate()
    {
        ...
        if (LastGenerateTime + GenerateDelay < Time.time)
        {
            ...
            Debug.DrawRay(RayGeneratePoint.position, direction, Color.red.gamma, 3.0f);     // 첫 Ray 3초간 시각화
    ...
    private IEnumerator GenerateTrail(TrailRenderer Trail, Vector3 ReflectingPoint, Vector3 ReflectingNormal, float ReflectingDistance, bool MadeImpact, string ColliderName)
    {
        ...
        if (ReflectingDistance <= 0)
        {
            ...
            distance = LastReflectingDistance - 1f;
            startingDistance = distance - 1f;
        ...
        if (MadeImpact)
        {
            ...
            if (ReflectingDistance > 0)
            {
                ...
                RaycastHit2D hitReflection = Physics2D.Raycast(ReflectingPoint - new Vector3(0.1f, 0.1f), ReflectingDirection, ReflectingDistance);
                ...
                else
                {
                    ...
                    RaycastHit2D hitReflectionLast = Physics2D.Raycast(ReflectingPoint - new Vector3(0.1f, 0.1f), ReflectingDirection, float.MaxValue);
